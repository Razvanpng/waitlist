...
export function DashboardPage() {
  ...
  const handleRestore = async (slotId: string) => {
    ...
    if (!slot) return;
    const status =
      slot.booked_count >= slot.capacity ? "booked" : "available";
      ...
      <main className="mx-auto max-w-5xl px-6 py-10 flex flex-col gap-10">
        ...
        <section className="border-2 border-foreground">
          ...
          >
            ...
            <Plus
              className={`h-5 w-5 transition-transform ${formOpen ? "rotate-45" : ""}`}
            />
          </button>
          ...
          {formOpen && (
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={() => { reset(); setFormOpen(false); setCreateError(null); }}
...
function SlotCard({
  ...
  return (
    <div
      className={`border-2 border-foreground ${STATUS_STYLES[slot.status]} bg-background`}
    >
